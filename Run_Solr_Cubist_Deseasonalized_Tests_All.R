    ...
    for(pentad in 1:num_groups) {
      ...
      if(nrow(test_data) > 0 && nrow(training_data) > 0) {
        if(!is.null(P_Deseasonalized_Solr[[as.character(pentad)]])) {
          ...
          P_Solr[[as.character(pentad)]][[as.character(test_year)]][["columns"]] <- as.vector(goodCorrelateTop(training_data, "SOLR_6", num_features, results=P_Deseasonalized_Solr[[as.character(pentad)]])[["names"]])
          P_Solr[[as.character(pentad)]][[as.character(test_year)]][["model"]] <- cubist(x=training_data[,P_Solr[[as.character(pentad)]][[as.character(test_year)]][["columns"]]], y=training_data[,"SOLR_6"])
          for(hour in 0:23) {
        ...
        if(!is.null(S_Deseasonalized_Solr[[as.character(pentad)]])) {
          ...
          S_Solr[[as.character(pentad)]][[as.character(test_year)]][["columns"]] <- as.vector(goodCorrelateTop(training_data, "SOLR_6", num_features, results=S_Deseasonalized_Solr[[as.character(pentad)]])[["names"]])
          S_Solr[[as.character(pentad)]][[as.character(test_year)]][["model"]] <- cubist(x=training_data[,S_Solr[[as.character(pentad)]][[as.character(test_year)]][["columns"]]], y=training_data[,"SOLR_6"])
          for(hour in 0:23) {
...
#           }
#           K_Solr[[as.character(pentad)]][[as.character(test_year)]][["columns"]] <- as.vector(goodCorrelateTop(training_data, "SOLR_6", num_features, results=K_Deseasonalized_Solr[[as.character(pentad)]])[["names"]])
#           K_Solr[[as.character(pentad)]][[as.character(test_year)]][["model"]] <- cubist(x=training_data[,K_Solr[[as.character(pentad)]][[as.character(test_year)]][["columns"]]], y=training_data[,"SOLR_6"])
#           for(hour in 0:23) {
...
return_dir <- getwd()
dir.create("Solr_Cubist_Deseasonalized_All_Results")
setwd("Solr_Cubist_Deseasonalized_All_Results")
...
for(i in 1:num_groups) {
  ...
  dir.create(pentad_dir)
  setwd(pentad_dir)
  writeRawTestResults(name=paste(station, "_", num_features,"_S_Solr_", "_Raw_Results.csv", sep=""), results=S_Solr, pentad=i)
  setwd("..")
}
...
save(result, file="Solr_Cubist_Deseasonalized_Single_Results.RData")