...
outputConsecutive <- function(data, cluster_name, file_name, type="kmeans") {
...
for(i in 4:10) {
  ...
  outputConsecutive(solr_data, kmeans_name, paste(kmeans_name, "_consecutive.csv", sep=""))
  
  haar_kmeans_name <- paste("coef_kmeans",i,sep="_")
  outputConsecutive(haar_solr_data, haar_kmeans_name, paste(haar_kmeans_name, "_consecutive.csv", sep=""))
  
  pam_name <- paste("pam",i,sep="_")
  outputConsecutive(solr_data, pam_name, paste(pam_name, "_consecutive.csv", sep=""), type="pam")
  
  haar_pam_name <- paste("coef_pam",i,sep="_")
  outputConsecutive(haar_solr_data, haar_pam_name, paste(haar_pam_name, "_consecutive.csv", sep=""), type="pam")
}